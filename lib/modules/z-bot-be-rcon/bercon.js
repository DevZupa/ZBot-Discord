  ...
  processPlayerList(text, channel) {
    ...
    lines.forEach((line) => {
      if (!line.includes('players in total') && !line.includes('player in total') ) {
        ...
        table.push([player.id, player.flag, player.name, player.ping,'secret','secret', player.guid]);

        message += '' + player.id + ' | ' + player.flag + ' | ' + player.name + ' | ' + player.ping + ' | ' + 'secret' + ' | ' + 'secret' + ' | ' + player.guid + ' \n'
